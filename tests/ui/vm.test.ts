...
describe('recommendVM — 출처 배지', () => {
  it("nuri → 누리과정 배지(gov)", () => {
    ...
    expect(vm).toMatchObject({ badgeCls: 'gov', sourceLabel: '누리과정', title: '그림책 함께 읽기', placeLabel: '집' })
  })
  it('achievement → 성취기준 배지(gov)', () => {
    expect(recommendVM({ ...base, source: 'achievement' })).toMatchObject({ badgeCls: 'gov', sourceLabel: '성취기준' })
  })
